...
{
	bool visited = false;
	float globalGoal; // goals as per A* alg, not used
...
int main()
{
	...
	// ===================================
	// ============== THESE VALUES CAN BE CHANGED: the grid size as well as the start/end positions
	static int gridSize = 100; // optional to change for different sized 'maps'
	static float initial_position[2] = {0, 0};
	static float final_position[2] = { gridSize, gridSize };
	static float constWaitTine = 10; // the constant wait time after reaching each node
	static float roboSpeed = 2; // in m/s
	// ===================================
	...
	{ //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> All input data is in a single line
		...
		for (int i = 1; i <= waypointNodes; i++)
		{
			...
			nodes[i].wait = constWaitTine; // all the checkponts have wait times
		}
	}
	else
	...
	nodes[0].x = initial_position[0];
	nodes[0].y = initial_position[1];
	nodes[0].penalty = 0;
	...
	nodes[waypointNodes + 1].x = final_position[0];
	nodes[waypointNodes + 1].y = final_position[1];
	nodes[waypointNodes + 1].penalty = 0;
	nodes[waypointNodes + 1].parent = nullptr;
	nodes[waypointNodes + 1].wait = constWaitTine;
	...
	for (int j = 0; j < nodeNumber; j++)
	{
		for (int k = j + 1; k < nodeNumber; k++)
		{
			...
			//cout << "........... to node: " << k << endl;
			float travTime = sqrt(pow((nodes[j].x - nodes[k].x), 2) + pow((nodes[j].y - nodes[k].y), 2)) / roboSpeed;
